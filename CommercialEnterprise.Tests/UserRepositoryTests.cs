    ...
    public class UserRepositoryTests
    {
        ...
        public void SetUp()
        {
            var options = new DbContextOptionsBuilder<EnterpriceContext>()
                .UseInMemoryDatabase("InMemory_Database")
                .Options;
        ...
        public async Task ChangePasswordByLoginAndKeyWordAsync_WhenCalled_ChangedPassword()
        {
            ...
            var actual = await context.Users.Where(x => x.Login == "login" && x.KeyWord == "qwerty").Select(x => x).AsNoTracking().FirstOrDefaultAsync();
