...
namespace CommercialEnterprise.Infrastructure.Repositories
{
    public class UserRepository : IUserRepository
    {
        ...
        {
            var user = await context.Users.Where(x => x.Login == login && x.KeyWord == keyWord).Select(x => x).FirstOrDefaultAsync();
        ...
        public async Task<User> GetUserByLoginAndPasswordAsync(string login, string password) =>
            await context.Users.Where(x => x.Login == login && x.Password == password).Select(x => x).FirstOrDefaultAsync();
